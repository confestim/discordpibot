...
async def on_ready():
    ...
    print(bot.user.id)
    print('------')
    bot.change_presence(game=discord.Game(name="?help"), status=discord.Status("dnd"))
...
async def coins():
    ...
    ethereum = requests.get("https://api.coinmarketcap.com/v1/ticker/ethereum/").json()
    dogecoin = requests.get("https://api.coinmarketcap.com/v1/ticker/dogecoin/").json()
    await bot.say('BTC: ${:.2f}\nETH: ${:.2f}\nDOGE: ${:.3f}'.format(float(bitcoin[0]["price_usd"]), float(ethereum[0]["price_usd"]), float(dogecoin[0]["price_usd"])))
